...
export function MobileShell({ children }: { children: ReactNode }) {
  ...
  if (isMobile) {
    return <div className="relative h-full w-full overflow-hidden bg-base">{children}</div>
  }

  return (
    <div className="flex min-h-screen items-center justify-center bg-[#04040A]">
      <div
        className="relative h-[844px] w-[390px] overflow-hidden rounded-[44px] border border-white/10 bg-[#07070F]"
        style={{
          boxShadow:
